...
static PT Deserialize(const std::string &str);
...
public:
    ...
    // 将优先队列最前面的一个PT
    void PopNext();
       void PopNextBatchMPI(int batch_size);

    // 处理PT批量任务
    void ProcessBatchMPI(std::vector<PT>& pt_batch);

    // 单个PT的密码生成辅助函数
    void GenerateGuessesForPT(PT& pt, std::vector<std::string>& output_guesses);
    int total_guesses = 0;
    vector<string> guesses;

    // 新增MPI版本的Generate函数
    void MPI_Generate(PT pt);
